...
productsListInfo.classList.add(
  "products-list-info",
  "d-flex",
  "flex-column",
  "p-2",
...
myOrdersButton.classList.add("btn", "btn-primary", "mb-2");
myOrdersButton.textContent = "My Orders";
myOrdersButton.addEventListener("click", showMyOrders);
categoriesList.appendChild(myOrdersButton);
...
function showMyOrders() {
  ...
  if (orders.length === 0) {
    const noOrdersLabel = document.createElement("p");
    noOrdersLabel.textContent = "No orders yet.";
    productsListInfo.appendChild(noOrdersLabel);
  } else {
    orders.forEach((order, index) => {
      const orderButton = document.createElement("button");
      orderButton.classList.add("btn", "btn-info", "mb-2");
      orderButton.textContent = `${order.product.name} Details:`;
      const deleteButton = createDeleteButton(order, index);
      const backButton = createBackButton();
      backButton.addEventListener("click", showCategoriesList);

      orderButton.addEventListener("click", () => showOrderDetails(order));

      productsListInfo.appendChild(orderButton);
      productsListInfo.appendChild(deleteButton);
      productsListInfo.appendChild(backButton);
    });
    displayTotalPrice();
  }
}
...
function createDeleteButton(order, index) {
...
function createBackButton() {
  ...
  backButton.addEventListener("click", showMyOrders);
  return backButton;
}
...
function displayTotalPrice() {
  ...
  totalContainer.textContent = `Total Price: $${calculateTotalPrice()}`;

  productsListInfo.appendChild(totalContainer);
}
...
function showOrderDetails(order) {
  ...
  const deleteButton = createDeleteButton(order.product);
  deleteButton.classList.add("mt-2");
  const backButton = createBackButton();
  backButton.classList.add("mt-2");
